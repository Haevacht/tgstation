...
/obj/item/weapon/twohanded/skybulge	//Copy+paste job from bonespear because no explosions.
	icon_state = "sky_bulge0" //Until I sprite up a suitable Gae Bolg
	name = "Sky Bulge"
	desc = "A legendary stick with a very pointy tip. Clone Please." //TODO: Be funnier.
	force = 10 //This thing aint for robusting if one handed.
	w_class = WEIGHT_CLASS_BULKY
	slot_flags = SLOT_BACK
	force_unwielded = 10
	force_wielded = 18					//Same as regular spear. This is a utility weapon.
	throwforce = 24						//And that utility is throwing. 24 so it takes 5 hits.
	throw_speed = 4
	...
	hitsound = 'sound/weapons/bladeslice.ogg'
	attack_verb = list("attacked", "poked", "jabbed", "torn", "gored", "lanced")
	sharpness = IS_SHARP

/obj/item/weapon/twohanded/skybulge/update_icon() //Ah fuck this means 2 sets of inhands.
		icon_state = "sky_bulge[wielded]"

/obj/item/weapon/twohanded/skybulge/throw_at()  //I had issues with this when 
	unwield(src)
	..()
